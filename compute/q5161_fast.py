...
def moments(T,N):
    ...
    S=[[0 for _ in range(N+1)] for _ in range(4)]
    for n in range(N+1):
        totals=[0,0,0,0]
...
cases=[]
cases.append(('direct A=3 J=0 degree=3',)+fit(3,0,3,False,False,31)[1:])
cases.append(('Gamma(n) A=3 J=0 degree=3',)+fit(3,0,3,True,False,31)[1:])
for J in (1,2):
    cases.append((f'direct shifts A=3 J={J} degree=3',)+fit(3,J,3,False,False,70)[1:])
    cases.append((f'Gamma(n) shifts A=3 J={J} degree=3',)+fit(3,J,3,True,False,70)[1:])
    cases.append((f'Gamma(n+j) shifts A=3 J={J} degree=3',)+fit(3,J,3,True,True,70)[1:])
